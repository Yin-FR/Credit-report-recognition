<template>
    <div>
        <ImgPart :index-rec="'one'"/>  <!--示例图片展示-->
        <Editor :index="0"/>
        <div>
            <el-button type="default" class="reserve-all-button" @click="recData(0)" v-show="this.$store.state.infoOrStatus[0].imagReceived">识别</el-button>  <!--识别按钮-->
        </div>
        <div v-if="this.$store.state.infoOrStatus[0].recFinishedStatus">   <!--识别结束后，展示结果-->
            <el-table :data="$store.state.recoginzeResults[0]" stripe border width="100%">
                <el-table-column prop="value.item" label="检测项" min-width="20%"></el-table-column>
                <el-table-column label="检测值" min-width="55%">
                    <template slot-scope="scope">
                        <el-input class="inputBlock" type="textarea" autosize v-model="scope.row.value.itemValue" :disabled="!scope.row.editable"></el-input>
                    </template>
                </el-table-column>
            ...
            <el-button type="success" class="reserve-all-button" @click="reserveAll(0)">保存</el-button>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            editLineHere(index){
                console.log(index);
                this.$store.dispatch('editLine', {indexImg: 0, indexLine: index})
            },

            reserveLineHere(index){
                console.log(index);
                this.$store.dispatch('reserveLine', {indexImg: 0, indexLine: index})
            },
